...
const debug = require('debug')('openapi-code-generator');
...
const handlebarsEngine = require('./handlebars-engine-openapi');
const handlebarsHelpersSwagger = require('./handlebars-helpers-openapi');
const FileTemplateLoader = require('./file-template-loader');
const oas3_templates = require('./oas3_template');
...
function generateCode(taskOptions) {
  try {
    ...
    const model = taskOptions.openapi;
    ...
    if (config.perDefinition) {
      ...
      for (const templateFile of Object.keys(config.perDefinition)) {
        ...
        for (const definitionKey of Object.keys(definitionMap)) {
          ...
          const subPath = path.join(options.target, definition.definitionName.trim().toLowerCase() + options.extension);
    ...
    if (config.perPath) {
      ...
      for (const templateFile of Object.keys(config.perPath)) {
        ...
        for (const groupKey of Object.keys(groups)) {
          ...
          const context = {};
          context.fileName = groupKey.trim();
          ...
          const output = template(context);
          const subPath = path.join(options.target, context.fileName.trim().toLowerCase() + options.extension);
          taskOptions.output(subPath, output);
        }
      } // per templateFile
    } // if perPath
